...
fn test_iq_conversion_dc_signal() {
    ...
    for sample in &chunk {
        assert!(
            sample.re.abs() < 0.01,
            "I component not near zero: {}",
            sample.re
        );
        assert!(
            sample.im.abs() < 0.01,
            "Q component not near zero: {}",
            sample.im
        );
    }
...
fn test_convert_cu8_boundaries() {
    ...
    let epsilon = 0.01;
    assert!(
        (chunk[0].re + 0.996).abs() < epsilon,
        "Sample 0 I: expected ~-0.996, got {}",
        chunk[0].re
    );
    assert!(
        (chunk[0].im + 0.996).abs() < epsilon,
        "Sample 0 Q: expected ~-0.996, got {}",
        chunk[0].im
    );
    assert!(
        (chunk[1].re + 0.004).abs() < epsilon,
        "Sample 1 I: expected ~-0.004, got {}",
        chunk[1].re
    );
    assert!(
        (chunk[1].im + 0.004).abs() < epsilon,
        "Sample 1 Q: expected ~-0.004, got {}",
        chunk[1].im
    );
    assert!(
        (chunk[2].re - 0.004).abs() < epsilon,
        "Sample 2 I: expected ~0.004, got {}",
        chunk[2].re
    );
    assert!(
        (chunk[2].im - 0.004).abs() < epsilon,
        "Sample 2 Q: expected ~0.004, got {}",
        chunk[2].im
    );
    assert!(
        (chunk[3].re - 0.996).abs() < epsilon,
        "Sample 3 I: expected ~0.996, got {}",
        chunk[3].re
    );
    assert!(
        (chunk[3].im - 0.996).abs() < epsilon,
...
fn test_convert_cs8_sign_handling() {
    ...
    let epsilon = 0.01;
    assert!(
        (chunk[0].re + 1.0).abs() < epsilon,
        "Sample 0 I: expected ~-1.0, got {}",
        chunk[0].re
    );
    assert!(
        (chunk[0].im + 1.0).abs() < epsilon,
        "Sample 0 Q: expected ~-1.0, got {}",
        chunk[0].im
    );
    assert!(
        (chunk[1].re + 0.0078).abs() < epsilon,
        "Sample 1 I: expected ~-0.0078, got {}",
        chunk[1].re
    );
    assert!(
        (chunk[1].im + 0.0078).abs() < epsilon,
        "Sample 1 Q: expected ~-0.0078, got {}",
        chunk[1].im
    );
    assert!(
        chunk[2].re.abs() < epsilon,
        "Sample 2 I: expected ~0.0, got {}",
        chunk[2].re
    );
    assert!(
        chunk[2].im.abs() < epsilon,
        "Sample 2 Q: expected ~0.0, got {}",
        chunk[2].im
    );
    assert!(
        (chunk[3].re - 0.992).abs() < epsilon,
        "Sample 3 I: expected ~0.992, got {}",
        chunk[3].re
    );
    assert!(
        (chunk[3].im - 0.992).abs() < epsilon,
...
fn test_convert_cs16_endianness() {
    ...
    assert!(
        chunk[1].re.abs() < epsilon,
        "Sample 1 I: expected ~0.0, got {}",
        chunk[1].re
    );
    assert!(
        chunk[1].im.abs() < epsilon,
        "Sample 1 Q: expected ~0.0, got {}",
        chunk[1].im
    );
    assert!(
        (chunk[2].re - 0.999).abs() < epsilon,
        "Sample 2 I: expected ~0.999, got {}",
        chunk[2].re
    );
    assert!(
        (chunk[2].im - 0.999).abs() < epsilon,
...
fn test_convert_cf32_float_precision() {
    ...
    let epsilon = 1e-6;
    assert!(
        (chunk[0].re - 0.5).abs() < epsilon,
        "Sample 0 I: expected 0.5, got {}",
        chunk[0].re
    );
    assert!(
        (chunk[0].im + 0.5).abs() < epsilon,
        "Sample 0 Q: expected -0.5, got {}",
        chunk[0].im
    );
    assert!(
        (chunk[1].re + 1.0).abs() < epsilon,
        "Sample 1 I: expected -1.0, got {}",
        chunk[1].re
    );
    assert!(
        (chunk[1].im - 1.0).abs() < epsilon,
        "Sample 1 Q: expected 1.0, got {}",
        chunk[1].im
    );
    assert!(
        chunk[2].re.abs() < epsilon,
